...
myawesomemenu = {
    ...
    { "edit theme", editor_cmd .. " " .. awful.util.getdir("config") .. "/themes/pabucolor/theme.lua", 
                                                            freedesktop.utils.lookup_icon({ icon = 'package_settings' }) },
    { "restart", awesome.restart, freedesktop.utils.lookup_icon({ icon = 'gtk-refresh' }) }
}

windowsmenu = {
    { "Powerpoint 2007", "playonlinux --run 'Microsoft Powerpoint 2007'" },
    { "Excel 2007",      "playonlinux --run 'Microsoft Excel 2007'" },
    { "Word 2007",       "playonlinux --run 'Microsoft Word 2007'" },
    { "Excel",           "excel" },
}
utilmenu = {
    ...
    { "Update directorios", "xdg-user-dirs-gtk-updater" },
...
--table.insert(menu_items, { "Salir Sesion", awesome.quit,      freedesktop.utils.lookup_icon({ icon = 'gtk-quit' })  })
...
--table.insert(menu_items, { "Reiniciar",   "shutdown -r now",  freedesktop.utils.lookup_icon({icon = 'view-refresh'})})table.insert(menu_items, { "Apagar",      "shutdown -h now",  freedesktop.utils.lookup_icon({icon = 'system-shutdown'})})
--table.insert(menu_items, { "Apagar",      "shutdown -h now",  freedesktop.utils.lookup_icon({icon = 'system-shutdown'})})
...
mytuxmenu = awful.menu({ items = {
              { "windows",  windowsmenu },
              { "utileria", utilmenu, beautiful.awesome2 },
              { "Awesome",  myawesomemenu, beautiful.awesome2 },
              { "Thunar",      "thunar",          freedesktop.utils.lookup_icon({icon = 'Thunar'})},
              { "spacefm",      "spacefm",          freedesktop.utils.lookup_icon({icon = 'spacefm'})},
              { "Pcmanfm",      "pcmanfm",          freedesktop.utils.lookup_icon({icon = 'Pcmanfm'})},
              { "Mocp",    terminalx .." -e mocp", freedesktop.utils.lookup_icon({icon = 'utilities-terminal'}) },
              { "Cmus",    terminalx .. " -e cmus", freedesktop.utils.lookup_icon({icon = 'utilities-terminal'}) },
              { "htop",    terminalx .. " -e htop", freedesktop.utils.lookup_icon({icon = 'htop'}) },
              { "tmux",    terminalx .. " -e tmux", freedesktop.utils.lookup_icon({icon = 'utilities-terminal'}) },
              { "Suspender",   "sudo pm-suspend",   freedesktop.utils.lookup_icon({icon = 'appointment-new'}) },
              { "Hibernar",    "sudo pm-hibernate", freedesktop.utils.lookup_icon({icon = 'document-open-recent'}) },
...
vol = lain.widgets.alsa({
    settings = function()
        if volume_now.status == " off" then
            ...
            level = markup("#FF0000", volume_now.level)
            --volicon = wibox.widget.imagebox(beautiful.icons_vol_mute)
            volicon:set_image(beautiful.icons_vol_mute)
        else
            level = markup("#F85EAC",  volume_now.level .. "%")
            --volicon = wibox.widget.imagebox(beautiful.icons_vol)
            volicon:set_image(beautiful.icons_vol)
        end
...
baticon = wibox.widget.imagebox()
  --bat = wibox.widget.textbox()
    --bat:set_text(batteryInfo("BAT1@"))
--bat = wibox.widget.textbox("BAT1")
...
bat = lain.widgets.bat({
    ...
    settings = function()
        if bat_now.perc == "N/A" then
          widget:set_markup(markup("#FF0000", " AC ") )
            perc = "AC"
        ...
        widget:set_markup(markup("#CF7BF0", " Bat: ") .. perc)
    end
})
...
mytextclock = awful.widget.textclock(markup("#7788af", " %A %d %B ") .. markup("#26CA34", "|") .. markup("#de5e1e", " %H:%M "))
...
-- MPD hol0
--audioicon = wibox.widget.imagebox(beautiful.icons_audio)
mpdicon = wibox.widget.imagebox()
mpdicon:set_image(beautiful.icons_audio)
...
mpdwidgetbg = lain.widgets.mpd({
    settings = function()
        if mpd_now.state == "play" then
            ...
            mpdicon:set_image(beautiful.icons_audio1)
...
--local volumewidget = wibox.widget.textbox()
    --vicious.register(volumewidget, vicious.widgets.volume,  
     --function (widget, args)
       --if (args[2] ~= "♩" ) then 
--         return '<span color="' .. "#7493d2" .. '"> Vol: </span>'
              --.. args[1] .. '%'
         
---       else r
         --return ' <span color="' .. "#0077FF" .. '">Vol:</spanr> <span color="' .. "#F7F100" .. '">Mute</span> '
        --end
     --end, 0.1, "Master")
...
--    awful.key({ modkey }, "p", function() menubar.show() end) -- Default M-p command, dmenu is a better/faster alternative
    ...
    --awful.key({altkey }, "XF86AudioRaiseVolume",   function () awful.util.spawn_with_shell("cmus-remote -v +3% && mcop -v +3%&")              end),
     ...
     awful.key({       }, "XF86AudioPlay",          function () awful.util.spawn_with_shell("cmus-remote -u || mocp -P || mpc toggle || ncmpcpp toggle || ncmpc toggle || pms toggle") end),
    ...
    awful.key({ modkey }, "a",    function () awful.util.spawn_with_shell("vlc --extraintf=luahttp --fullscreen  --http-password elena2006 --http-host 192.168.1.50 --http-port 8801 --qt-start-minimized")         end),
 ...
 --awful.key({ modkey }, "y",    function () awful.util.spawn_with_shell( cfg_dir .. "/scripts/xephyr.sh") end),
    ...
    awful.key({ modkey }, "F6",   function () awful.util.spawn("killall ffmpeg && killall mpd &  killall ncmpcpp")        end),
...
autostart("conky", 2)
autostart("sh ~/.config/awesome/themes/pabucolor/wallpaper.sh", 1)
